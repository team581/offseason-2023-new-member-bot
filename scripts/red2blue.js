...
// Default to all red paths
if (filenames.length === 0) {
  const dir = await fs.readdir(pathDir);

  filenames = dir
    .filter(
      (filename) => filename.startsWith("Red") && filename.endsWith(".path")
    )
    .map((absolutePath) => nodePath.basename(absolutePath));
}

if (filenames.some(filename => filename.startsWith("Blue"))) {
  throw new RangeError('Some of the filenames are blue paths, not red ones');
}
...
for (const filename of filenames) {
  ...
  const outputPath = nodePath.join(
    pathDir,
    `Blue${filename.slice("Red".length)}`
  );
